...
int main() {
    ...
    if (!text || *text != "hello filesystem" || !bytes || bytes->size() != binary.size() ||
        !std::ranges::equal(*bytes, binary)) {
        return 5;
    }
    ...
    const std::vector<VirtualPath> files = fileSystem.listFiles(VirtualPath{"test://"}, true);
    if (files.size() != 2 || std::ranges::find_if(files, [&textPath](const VirtualPath& path) {
    ...
    if (!physical ||
        physical->lexically_normal() != (root / "folder/message.txt").lexically_normal()) {
        return 8;
    }
    const VirtualPath native = VirtualPath::fromNative(*physical);
    const auto nativeText = fileSystem.readText(native);
    if (!native.valid() || !native.isNative() || !nativeText || *nativeText != "hello filesystem") {
        return 9;
    }
    ...
    if (!atomicStat || !atomicStat->isFile || atomicStat->size != 6 || !virtualPath ||
        virtualPath->string() != atomicPath.string()) {
        return 13;
    }
    if (!fileSystem.createDirectories(VirtualPath{"test://moved"}) ||
        !fileSystem.move(atomicPath, movedPath) || fileSystem.exists(atomicPath) ||
        !fileSystem.exists(movedPath) || !fileSystem.removeFile(movedPath) ||
        fileSystem.exists(movedPath)) {
        return 14;
    }
