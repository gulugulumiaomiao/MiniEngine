...
GeneratedMesh makePlane(const PlaneGeometry& geometry) {
    ...
    mesh.vertices.reserve(static_cast<std::size_t>(width) *
                          (geometry.segmentsZ + 1));
    for (std::uint32_t z = 0; z <= geometry.segmentsZ; ++z) {
        ...
        for (std::uint32_t x = 0; x <= geometry.segmentsX; ++x) {
            ...
            mesh.vertices.push_back({{(u - 0.5F) * geometry.size.x, 0.0F,
                                      (v - 0.5F) * geometry.size.y},
                                     {0.0F, 1.0F, 0.0F},
                                     {1.0F, 0.0F, 0.0F, 1.0F},
                                     {u, v}});
        }
...
void appendFace(GeneratedMesh& mesh, const math::Vec3& center,
                const math::Vec3& uAxis, const math::Vec3& vAxis,
                const math::Vec3& normal, std::uint32_t uSegments,
    ...
    for (std::uint32_t y = 0; y <= vSegments; ++y) {
        ...
        for (std::uint32_t x = 0; x <= uSegments; ++x) {
            ...
            mesh.vertices.push_back(
                {center + (u - 0.5F) * uAxis + (v - 0.5F) * vAxis,
                 normal,
                 {tangent, 1.0F},
                 {u, v}});
        }
...
GeneratedMesh makeBox(const BoxGeometry& geometry) {
    ...
    appendFace(mesh, {half.x, 0, 0}, {0, 0, -geometry.size.z},
               {0, geometry.size.y, 0}, {1, 0, 0}, geometry.segmentsZ,
               geometry.segmentsY);
    appendFace(mesh, {-half.x, 0, 0}, {0, 0, geometry.size.z},
               {0, geometry.size.y, 0}, {-1, 0, 0}, geometry.segmentsZ,
               geometry.segmentsY);
    appendFace(mesh, {0, half.y, 0}, {geometry.size.x, 0, 0},
               {0, 0, geometry.size.z}, {0, 1, 0}, geometry.segmentsX,
               geometry.segmentsZ);
    appendFace(mesh, {0, -half.y, 0}, {geometry.size.x, 0, 0},
               {0, 0, -geometry.size.z}, {0, -1, 0}, geometry.segmentsX,
               geometry.segmentsZ);
    appendFace(mesh, {0, 0, half.z}, {geometry.size.x, 0, 0},
               {0, geometry.size.y, 0}, {0, 0, 1}, geometry.segmentsX,
               geometry.segmentsY);
    appendFace(mesh, {0, 0, -half.z}, {-geometry.size.x, 0, 0},
               {0, geometry.size.y, 0}, {0, 0, -1}, geometry.segmentsX,
               geometry.segmentsY);
...
GeneratedMesh makeSphere(const UvSphereGeometry& geometry) {
    ...
    for (std::uint32_t latitude = 0; latitude <= geometry.latitudeSegments;
         ++latitude) {
        const float v =
            static_cast<float>(latitude) / geometry.latitudeSegments;
        const float theta = v * math::kPi;
        const float ring = std::sin(theta);
        const float y = std::cos(theta);
        for (std::uint32_t longitude = 0;
             longitude <= geometry.longitudeSegments; ++longitude) {
            const float u =
                static_cast<float>(longitude) / geometry.longitudeSegments;
            const float phi = u * math::kPi * 2.0F;
            const math::Vec3 normal{ring * std::cos(phi), y,
                                    ring * std::sin(phi)};
            const math::Vec3 tangent =
                math::normalize(math::Vec3{-std::sin(phi), 0.0F, std::cos(phi)},
                                math::Vec3{1, 0, 0});
            mesh.vertices.push_back(
                {normal * geometry.radius, normal, {tangent, 1.0F}, {u, v}});
        }
    }
    for (std::uint32_t latitude = 0; latitude < geometry.latitudeSegments;
         ++latitude) {
...
void appendCap(GeneratedMesh& mesh, float radius, float y,
...
GeneratedMesh makeCylinder(const CylinderGeometry& geometry) {
    ...
    const float slope =
        (geometry.bottomRadius - geometry.topRadius) / geometry.height;
    for (std::uint32_t height = 0; height <= geometry.heightSegments;
         ++height) {
        const float v = static_cast<float>(height) / geometry.heightSegments;
        const float y = -halfHeight + v * geometry.height;
        const float radius =
            math::lerp(geometry.bottomRadius, geometry.topRadius, v);
        for (std::uint32_t radial = 0; radial <= geometry.radialSegments;
             ++radial) {
            const float u =
                static_cast<float>(radial) / geometry.radialSegments;
            const float angle = u * math::kPi * 2.0F;
            const float x = std::cos(angle);
            const float z = std::sin(angle);
            ...
            mesh.vertices.push_back(
                {{x * radius, y, z * radius}, normal, {tangent, 1.0F}, {u, v}});
        }
    }
    ...
    if (geometry.capBottom)
        appendCap(mesh, geometry.bottomRadius, -halfHeight,
                  geometry.radialSegments, false);
    if (geometry.capTop)
        appendCap(mesh, geometry.topRadius, halfHeight, geometry.radialSegments,
                  true);
    orientClockwise(mesh);
    return mesh;
}
    ...
    return std::visit(
        [](const auto& geometry) -> std::optional<GeneratedMesh> {
            ...
            if constexpr (std::is_same_v<Geometry, PlaneGeometry>) {
                if (!finite(geometry.size) || geometry.size.x <= 0 ||
                    geometry.size.y <= 0 ||
                    !validSegments(geometry.segmentsX) ||
                    !validSegments(geometry.segmentsZ))
                    return std::nullopt;
                return makePlane(geometry);
            } else if constexpr (std::is_same_v<Geometry, BoxGeometry>) {
                if (!finite(geometry.size) || geometry.size.x <= 0 ||
                    geometry.size.y <= 0 || geometry.size.z <= 0 ||
                    !validSegments(geometry.segmentsX) ||
                    !validSegments(geometry.segmentsY) ||
                    !validSegments(geometry.segmentsZ))
                    return std::nullopt;
                ...
                if (!finite(geometry.bottomRadius) ||
                    !finite(geometry.topRadius) || !finite(geometry.height) ||
                    geometry.bottomRadius < 0 || geometry.topRadius < 0 ||
                    (geometry.bottomRadius <= math::kEpsilon &&
                     geometry.topRadius <= math::kEpsilon) ||
                    geometry.height <= 0 ||
                    !validSegments(geometry.radialSegments, 3) ||
...
VertexLayout layoutFor(PrimitiveVertexLayout preset) {
    ...
    layout.attributes.push_back({{VertexSemanticType::Position, 0},
                                 VertexFormat::Vec3Float32,
                                 0,
                                 0,
                                 0});
    if (preset != PrimitiveVertexLayout::Position) {
        layout.attributes.push_back({{VertexSemanticType::Normal, 0},
                                     VertexFormat::Vec3Float32,
                                     1,
                                     0,
                                     12});
        if (preset == PrimitiveVertexLayout::PositionNormalTangentUv) {
            layout.attributes.push_back({{VertexSemanticType::Tangent, 0},
                                         VertexFormat::Vec4Float32,
                                         2,
                                         0,
                                         24});
            layout.attributes.push_back({{VertexSemanticType::TexCoord, 0},
                                         VertexFormat::Vec2Float32,
                                         3,
                                         0,
                                         40});
            stride = 48;
...
void writeValue(std::vector<std::byte>& bytes, std::size_t offset,
...
std::optional<MeshBuildResult>
MeshBuilder::build(const MeshBuildRecipe& recipe) {
    if (recipe.parts.empty() || recipe.parts.size() > 4096) {
        Log::error("MeshBuilder",
                   "A build recipe must contain between 1 and 4096 parts");
        return std::nullopt;
    }
    if (recipe.vertexLayout < PrimitiveVertexLayout::Position ||
        ...
        recipe.usage < MeshUsage::Static || recipe.usage > MeshUsage::Stream) {
    ...
    for (const MeshPrimitivePart& part : recipe.parts) {
        if (!finite(part.translation) || !finite(part.scale) ||
            !finite(part.rotation) ||
            std::abs(part.scale.x) <= math::kEpsilon ||
            std::abs(part.scale.y) <= math::kEpsilon ||
            std::abs(part.scale.z) <= math::kEpsilon ||
            math::lengthSquared(part.rotation) <=
                math::kEpsilon * math::kEpsilon) {
            Log::error("MeshBuilder",
                       "A primitive part has an invalid transform");
            return std::nullopt;
        }
        auto generated = generate(part.primitive);
        if (!generated || generated->vertices.empty() ||
            generated->indices.empty()) {
            Log::error("MeshBuilder",
                       "A primitive part has invalid parameters");
            return std::nullopt;
        }
        if (vertices.size() + generated->vertices.size() >
            kMaxGeneratedVertices) {
        ...
        const math::Mat44 transform = math::trs(
            part.translation, math::normalize(part.rotation), part.scale);
        ...
        for (GeneratedVertex vertex : generated->vertices) {
            ...
            vertex.normal = math::normalize(normalTransform * vertex.normal,
                                            math::Vec3{0, 1, 0});
            math::Vec3 tangent =
                math::transformVector(transform, math::Vec3{vertex.tangent});
            tangent = math::normalize(
                tangent - vertex.normal * math::dot(vertex.normal, tangent),
                math::Vec3{1, 0, 0});
        ...
        subMeshes.push_back(
            {firstIndex, static_cast<std::uint32_t>(generated->indices.size()),
             0, part.materialSlot, calculateBounds(partPositions)});
    }
    ...
    if (recipe.indexPolicy == MeshIndexPolicy::UInt16 ||
        ...
        if (vertices.size() > static_cast<std::size_t>(
                                  ...
                                  1U) {
            Log::error("MeshBuilder",
                       "The recipe exceeds the UInt16 index range");
            return std::nullopt;
        }
    ...
    const std::uint32_t stride =
        result.desc.vertexLayout.bindings.front().stride;
    ...
    if (!result.data.setVertexData(
            0, static_cast<std::uint32_t>(vertices.size()), vertexBytes))
        return std::nullopt;
...
MeshBuilder::buildAsset(const MeshBuildRecipe& recipe) {
