...
bool PlaneGeometry::transfer(Transfer& archive) {
    return archive.transfer("size", size) &&
           archive.transfer("segments_x", segmentsX) &&
...
bool BoxGeometry::transfer(Transfer& archive) {
    return archive.transfer("size", size) &&
           archive.transfer("segments_x", segmentsX) &&
           archive.transfer("segments_y", segmentsY) &&
           archive.transfer("segments_z", segmentsZ);
}
...
bool CylinderGeometry::transfer(Transfer& archive) {
    return archive.transfer("bottom_radius", bottomRadius) &&
           archive.transfer("top_radius", topRadius) &&
           archive.transfer("height", height) &&
           archive.transfer("radial_segments", radialSegments) &&
           archive.transfer("height_segments", heightSegments) &&
           archive.transfer("cap_bottom", capBottom) &&
           archive.transfer("cap_top", capTop);
}
...
bool MeshPrimitive::transfer(Transfer& archive) {
    ...
    if (archive.reading()) {
        ...
        case MeshPrimitiveType::UvSphere:
            value.emplace<UvSphereGeometry>();
            break;
        case MeshPrimitiveType::Cylinder:
            value.emplace<CylinderGeometry>();
            break;
        default: return false;
        }
    }
    return std::visit(
        [&archive](auto& geometry) {
            return archive.transfer("parameters", geometry);
        },
        value);
}
...
bool MeshPrimitivePart::transfer(Transfer& archive) {
    return archive.transfer("primitive", primitive) &&
           archive.transfer("translation", translation) &&
