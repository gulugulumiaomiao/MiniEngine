...
std::uint64_t PipelineCache::makeKey(const ShaderProgram& program,
    ...
    for (const VertexBinding& binding : vertexLayout.bindings) {
        append(binding.binding); append(binding.stride); append(binding.inputRate);
    }
    for (const VertexAttribute& attribute : vertexLayout.attributes) {
        append(attribute.location); append(attribute.offset);
        append(attribute.binding); append(attribute.format);
        append(attribute.semantic.type); append(attribute.semantic.index);
    }
    return key;
}

std::uint64_t PipelineCache::makeFallbackKey(
    ...
    auto append = [&key](const auto& value) {
        key = hashBytes(
            {reinterpret_cast<const std::byte*>(&value), sizeof(value)}, key);
    };
    append(pass.type());
    ...
    if (!programHandle) {
        if (const auto previous = fallbackEntries_.find(fallbackKey);
            ...
            if (slot.alive && slot.pipeline) {
                ...
                return {previous->second, slot.generation};
            }
        }
...
void PipelineCache::invalidate(std::span<const CompiledShaderId> shaders,
    ...
    for (auto entry = entries_.begin(); entry != entries_.end();) {
        ...
        std::erase_if(fallbackEntries_, [index = entry->second](const auto& item) {
