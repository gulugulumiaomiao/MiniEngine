...
public:
    ...
    static void info(std::string_view source, const char* format, Args&&... args) {
        write(LogLevel::Info, source, formatMessage(format, std::forward<Args>(args)...));
    }
    ...
    static void debug(std::string_view source, const char* format, Args&&... args) {
        write(LogLevel::Debug, source, formatMessage(format, std::forward<Args>(args)...));
    }
    ...
    static void warn(std::string_view source, const char* format, Args&&... args) {
        write(LogLevel::Warn, source, formatMessage(format, std::forward<Args>(args)...));
    }
    ...
    static void error(std::string_view source, const char* format, Args&&... args) {
        write(LogLevel::Error, source, formatMessage(format, std::forward<Args>(args)...));
    }
    [[noreturn]] static void fatal(std::string_view source, std::string_view message);
    template <typename... Args>
        requires(sizeof...(Args) > 0)
    [[noreturn]] static void fatal(std::string_view source, const char* format, Args&&... args) {
        write(LogLevel::Fatal, source, formatMessage(format, std::forward<Args>(args)...));
...
private:
    ...
    static std::string formatMessage(const char* format, Args&&... args) {
        const int length = std::snprintf(nullptr, 0, format, std::forward<Args>(args)...);
        if (length < 0) {
    ...
    static void write(LogLevel level, std::string_view source, std::string_view message);
};
