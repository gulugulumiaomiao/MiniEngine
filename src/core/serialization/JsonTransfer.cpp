...
Json* selectWrite(Json& root, std::vector<Json*>& stack, std::string_view name) {
    if (stack.empty())
        return &root;
...
const Json*
selectRead(const Json& root, const std::vector<const Json*>& stack, std::string_view name) {
    if (stack.empty())
        return &root;
...
template <typename T> bool readSigned(const Json* source, T& value) {
    if (!source || (!source->is_number_integer() && !source->is_number_unsigned()))
        return false;
...
template <typename T> bool readUnsigned(const Json* source, T& value) {
    if (!source || !source->is_number_unsigned())
        return false;
    const std::uint64_t encoded = source->get<std::uint64_t>();
    if (encoded > static_cast<std::uint64_t>(std::numeric_limits<T>::max())) {
        return false;
...
bool JsonWriter::endObject() {
    if (impl_->stack.empty())
        return fail("JSON object stack is empty");
...
bool JsonWriter::endArrayElement() {
    return endObject();
}
...
bool JsonWriter::transferFloat(std::string_view name, float& value) {
    if (!std::isfinite(value))
        return fail("Cannot write a non-finite float");
    *selectWrite(impl_->root, impl_->stack, name) = value;
    return true;
}
...
bool JsonWriter::transferDouble(std::string_view name, double& value) {
    if (!std::isfinite(value))
        return fail("Cannot write a non-finite double");
    *selectWrite(impl_->root, impl_->stack, name) = value;
    return true;
}

bool JsonWriter::transferBytes(std::string_view name, std::vector<std::byte>& value) {
...
JsonReader::JsonReader(std::string_view source) : impl_(std::make_unique<Impl>()) {
    ...
    if (impl_->root.is_discarded())
        fail("Invalid JSON document");
}
...
bool JsonReader::beginObject(std::string_view name) {
    ...
    if (!value || !value->is_object())
        return fail("JSON object is missing or invalid");
    impl_->stack.push_back(value);
    return true;
}
...
bool JsonReader::endArrayElement() {
    return endObject();
}
...
bool JsonReader::transferBool(std::string_view name, bool& value) {
    ...
    if (!source || !source->is_boolean())
        return fail("JSON bool is missing or invalid");
...
bool JsonReader::transferFloat(std::string_view name, float& value) {
    const Json* source = selectRead(impl_->root, impl_->stack, name);
    if (!source || !source->is_number())
        return fail("JSON float is missing or invalid");
...
bool JsonReader::transferDouble(std::string_view name, double& value) {
    const Json* source = selectRead(impl_->root, impl_->stack, name);
    if (!source || !source->is_number())
        return fail("JSON double is missing or invalid");
...
bool JsonReader::transferString(std::string_view name, std::string& value) {
    ...
    if (!source || !source->is_string())
        return fail("JSON string is missing or invalid");
    value = source->get<std::string>();
    return true;
}
bool JsonReader::transferBytes(std::string_view name, std::vector<std::byte>& value) {
    const Json* source = selectRead(impl_->root, impl_->stack, name);
    ...
    for (const Json& element : *source) {
        std::uint8_t byte{};
        if (!readUnsigned(&element, byte))
            return fail("JSON byte is invalid");
