...
bool Transfer::transfer(std::string_view name, VirtualPath& value) {
    ...
    if (reading()) {
        ...
        if (!value.valid())
            return fail("Virtual path is invalid");
    }
    return true;
}
...
bool Transfer::transfer(std::string_view name, math::Vec2& value) {
    std::uint32_t size = 2;
    if (!beginArray(name, size) || size != 2)
        return fail("Vec2 size is invalid");
    for (std::uint32_t index = 0; index < size; ++index) {
        if (!beginArrayElement(index) || !transfer({}, value[index]) || !endArrayElement())
            return false;
    }
    return endArray();
}
...
bool Transfer::transfer(std::string_view name, math::Vec3& value) {
    std::uint32_t size = 3;
    if (!beginArray(name, size) || size != 3)
        return fail("Vec3 size is invalid");
    for (std::uint32_t index = 0; index < size; ++index) {
        if (!beginArrayElement(index) || !transfer({}, value[index]) || !endArrayElement())
            return false;
    }
...
bool Transfer::transfer(std::string_view name, math::Vec4& value) {
    std::uint32_t size = 4;
    if (!beginArray(name, size) || size != 4)
        return fail("Vec4 size is invalid");
    for (std::uint32_t index = 0; index < size; ++index) {
        if (!beginArrayElement(index) || !transfer({}, value[index]) || !endArrayElement())
            return false;
    }
...
bool Transfer::transfer(std::string_view name, math::Mat33& value) {
    std::uint32_t size = 9;
    if (!beginArray(name, size) || size != 9)
        return fail("Mat33 size is invalid");
    for (std::uint32_t index = 0; index < size; ++index) {
        if (!beginArrayElement(index) || !transfer({}, value[index / 3][index % 3]) ||
            !endArrayElement())
            return false;
    }
    return endArray();
...
bool Transfer::transfer(std::string_view name, math::Mat44& value) {
    ...
    if (!beginArray(name, size) || size != 16)
        return fail("Mat44 size is invalid");
    for (std::uint32_t index = 0; index < size; ++index) {
        if (!beginArrayElement(index) || !transfer({}, value[index / 4][index % 4]) ||
            !endArrayElement())
            return false;
    }
    return endArray();
...
bool Transfer::transfer(std::string_view name, math::Quat& value) {
    ...
    if (!transfer(name, encoded))
        return false;
    if (reading())
        value = math::Quat{encoded.w, encoded.x, encoded.y, encoded.z};
    return true;
}
