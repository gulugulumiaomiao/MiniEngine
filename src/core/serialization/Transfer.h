...
public:
    ...
    bool transfer(std::string_view name, T& value) {
        ...
        if (reading()) value = static_cast<T>(encoded);
        return true;
    }
    ...
    template <typename T>
    bool transfer(std::string_view name, std::vector<T>& values) {
        std::uint32_t size = writing()
            ? static_cast<std::uint32_t>(values.size()) : 0;
        if (writing() && values.size() > maxCollectionSize()) {
            return fail("Collection is too large");
        }
        if (!beginArray(name, size) || size > maxCollectionSize()) return false;
        if (reading()) values.resize(size);
        for (std::uint32_t index = 0; index < size; ++index) {
            if (!beginArrayElement(index) ||
                !transfer({}, values[index]) || !endArrayElement()) {
                return false;
            }
        }
        return endArray();
    ...
    template <typename T>
    bool transfer(std::string_view name, std::optional<T>& value) {
        if (!beginObject(name)) return false;
        bool present = writing() && value.has_value();
        if (!transfer("has_value", present)) return false;
        if (reading()) {
            if (present) value.emplace();
            else value.reset();
        }
        if (present && !transfer("value", *value)) return false;
        return endObject();
    }
    ...
    bool transfer(std::string_view name, std::variant<Types...>& value) {
        if (!beginObject(name)) return false;
        std::uint32_t index = writing()
            ? static_cast<std::uint32_t>(value.index()) : 0;
        if (!transfer("type", index)) return false;
        if (index >= sizeof...(Types)) return fail("Variant type is invalid");
        if (reading() && !emplaceVariant<0>(value, index)) {
            return fail("Variant type is invalid");
        }
        const bool result = std::visit(
            [this](auto& item) { return this->transfer("value", item); }, value);
        return result && endObject();
    }

    [[nodiscard]] virtual std::uint32_t maxCollectionSize() const {
        return 1U << 20U;
    }
...
private:
    ...
    virtual bool transferBytes(std::string_view name,
...
public:
    [[nodiscard]] TransferMode mode() const final {
        return TransferMode::Read;
    }
};
