...
#include "core/filesystem/IFileMount.h"
...
public:
    [[nodiscard]] bool mount(std::string_view scheme, std::shared_ptr<IFileMount> fileMount);
    ...
    [[nodiscard]] std::optional<std::string> readText(const VirtualPath& path) const;
    [[nodiscard]] std::optional<std::vector<std::byte>> readBinary(const VirtualPath& path) const;
    [[nodiscard]] bool writeText(const VirtualPath& path, std::string_view content);
    [[nodiscard]] bool writeBinary(const VirtualPath& path, std::span<const std::byte> content);
    [[nodiscard]] bool writeTextAtomic(const VirtualPath& path, std::string_view content);
    [[nodiscard]] bool writeBinaryAtomic(const VirtualPath& path,
...
private:
    ...
    [[nodiscard]] std::optional<MountLookup> lookup(const VirtualPath& path,
                                                    bool reportFailure = true) const;
