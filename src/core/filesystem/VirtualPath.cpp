...
bool validScheme(std::string_view scheme) {
    ...
    return std::ranges::all_of(scheme, [](char character) {
        ...
        return std::isalnum(value) != 0 || character == '+' || character == '-' || character == '_';
    });
}
...
VirtualPath::VirtualPath(std::string_view path) {
    ...
    if (scheme_ == "file") {
        ...
        std::ranges::replace(native, '/', std::filesystem::path::preferred_separator);
        const std::filesystem::path normalized = std::filesystem::path{native}.lexically_normal();
...
std::filesystem::path VirtualPath::nativePath() const {
    return isNative() && valid_ ? std::filesystem::path{relativePath_} : std::filesystem::path{};
}
...
VirtualPath VirtualPath::parent() const {
    ...
    if (isNative())
        return fromNative(path.parent_path());
...
VirtualPath VirtualPath::joined(std::string_view relativePath) const {
    if (!valid_)
        return {};
    if (VirtualPath absolute{relativePath}; absolute.valid())
        return absolute;
    const std::filesystem::path joined =
        (std::filesystem::path{relativePath_} / std::filesystem::path{relativePath})
            .lexically_normal();
    if (isNative())
        return fromNative(joined);
...
std::string VirtualPath::extension() const {
    return valid_ ? std::filesystem::path{relativePath_}.extension().string() : std::string{};
}
