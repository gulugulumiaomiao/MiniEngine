...
DirectoryMount::DirectoryMount(std::filesystem::path root, bool readOnly) : readOnly_(readOnly) {
...
DirectoryMount::resolvePhysicalPath(std::string_view relativePath) const {
    ...
    const std::filesystem::path relative = std::filesystem::path{relativePath}.lexically_normal();
...
std::optional<FileStat> DirectoryMount::stat(std::string_view relativePath) const {
    const auto path = resolvePhysicalPath(relativePath);
    if (!path) {
        return std::nullopt;
    }
...
bool DirectoryMount::writeBinary(std::string_view relativePath, std::span<const std::byte> data) {
    if (readOnly_) {
        return false;
    }
    const auto path = resolvePhysicalPath(relativePath);
...
#ifdef _WIN32
    const bool replaced = MoveFileExW(temporary.c_str(),
                                      destination->c_str(),
    ...
    if (recursive) {
        ...
        while (!error && iterator != end) {
            if (iterator->is_regular_file(error) && !error) {
                result.push_back(iterator->path().lexically_relative(root_).generic_string());
            }
