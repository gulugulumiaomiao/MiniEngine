...
bool FileSystem::mount(std::string_view scheme, std::shared_ptr<IFileMount> fileMount) {
    const std::string normalized = normalizeScheme(scheme);
    if (normalized.empty() || !fileMount) {
        Log::error("FileSystem",
                   "Cannot mount invalid scheme: %.*s",
                   static_cast<int>(scheme.size()),
                   scheme.data());
        return false;
    }
...
bool FileSystem::mountDirectory(std::string_view scheme,
    ...
    if (!directoryMount->valid()) {
        Log::error("FileSystem", "Cannot mount directory: %s", directory.string().c_str());
        return false;
    }
...
std::optional<FileSystem::MountLookup> FileSystem::lookup(const VirtualPath& path,
    ...
    if (!path.valid()) {
        if (reportFailure) {
            Log::error("FileSystem", "Invalid virtual path: %s", path.string().c_str());
        }
        return std::nullopt;
    }
    if (path.isNative()) {
        ...
        if (root.empty() || relative.empty()) {
            if (reportFailure) {
                Log::error("FileSystem", "Invalid native path: %s", path.string().c_str());
            }
            return std::nullopt;
        }
        return MountLookup{std::make_shared<DirectoryMount>(root), relative.generic_string()};
    }
    ...
    if (!fileMount) {
        if (reportFailure) {
            Log::error("FileSystem", "Scheme is not mounted: %s", path.scheme().c_str());
        }
        return std::nullopt;
    }
...
std::optional<std::vector<std::byte>> FileSystem::readBinary(const VirtualPath& path) const {
...
bool FileSystem::writeBinary(const VirtualPath& path, std::span<const std::byte> content) {
    const auto found = lookup(path);
    if (!found) {
        return false;
    }
    if (found->mount->readOnly()) {
        Log::error("FileSystem", "Cannot write to read-only mount: %s", path.string().c_str());
        return false;
    }
...
bool FileSystem::writeText(const VirtualPath& path, std::string_view content) {
    return writeBinary(path, {reinterpret_cast<const std::byte*>(content.data()), content.size()});
}

bool FileSystem::writeBinaryAtomic(const VirtualPath& path, std::span<const std::byte> content) {
    const auto found = lookup(path);
    if (!found || found->mount->readOnly()) {
        Log::error("FileSystem", "Cannot atomically write file: %s", path.string().c_str());
        return false;
    }
...
bool FileSystem::writeTextAtomic(const VirtualPath& path, std::string_view content) {
    return writeBinaryAtomic(path,
...
bool FileSystem::createDirectories(const VirtualPath& path) {
    ...
    if (!found || found->mount->readOnly() ||
        ...
        Log::error("FileSystem", "Cannot create directory: %s", path.string().c_str());
        return false;
    }
    return true;
}
...
bool FileSystem::removeFile(const VirtualPath& path) {
    const auto found = lookup(path);
    if (!found || found->mount->readOnly() || !found->mount->removeFile(found->relativePath)) {
...
std::vector<VirtualPath> FileSystem::listFiles(const VirtualPath& directory, bool recursive) const {
    std::vector<VirtualPath> result;
    const auto found = lookup(directory);
    if (!found) {
        return result;
    }
    const std::vector<std::string> files = found->mount->listFiles(found->relativePath, recursive);
...
FileSystem::resolvePhysicalPath(const VirtualPath& path) const {
    const auto found = lookup(path);
    return found ? found->mount->resolvePhysicalPath(found->relativePath) : std::nullopt;
}
