...
VirtualPath readAssetPath(const VirtualPath& owner,
    ...
    const AssetReference reference{value};
    if (reference.isGuid()) {
        if (!resolver) {
            return VirtualPath{};
        }
        const auto resolved = reference.resolve(*resolver);
        return resolved ? *resolved : VirtualPath{};
    }
    if (reference.isPath()) {
...
bool parseComponent(const VirtualPath& path,
                    const Json& source,
    ...
    if (typeName == "Mesh") {
        ...
        if (mesh != source.end()) {
            ...
            value.mesh = readAssetPath(path, mesh->get_ref<const std::string&>(), resolver);
        } else {
    ...
    if (typeName == "Material") {
        ...
        for (const Json& material : *materials) {
            if (!material.is_string())
                return false;
            value.materials.push_back(
                readAssetPath(path, material.get_ref<const std::string&>(), resolver));
        }
        result = std::move(value);
        return true;
    }
...
std::shared_ptr<SceneAsset> parseSceneAssetImpl(const VirtualPath& path,
    ...
    for (const Json& sourceNode : *nodes) {
        ...
        for (const Json& sourceComponent : *components) {
            ...
            if (!parseComponent(path, sourceComponent, component, resolver)) {
...
std::string writeSceneAssetJson(const SceneAsset& asset, const GuidResolver& resolver) {
    Json root;
    root["$schemaVersion"] = kSceneJsonVersion;
                ...
                [&](const auto& value) {
                    ...
                    if constexpr (std::is_same_v<T, TransformComponentAsset>) {
                        ...
                        if (value.sourceType == MeshComponentSourceType::Asset) {
                            const auto meshGuid = resolver.findGuid(value.mesh);
                        ...
                        for (const VirtualPath& material : value.materials) {
                            const auto materialGuid = resolver.findGuid(material);
