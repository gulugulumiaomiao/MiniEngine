...
bool AssetManager::initialize() {
    ...
    if (!FILE_SYSTEM.isDirectory(VirtualPath{"asset://"})) {
        Log::error("AssetManager",
                   "asset:// must be mounted before initialization");
        return false;
    }
    ...
    ASSET_IMPORT_PIPELINE.setListener(
        [this](const AssetImportNotification& notification) {
            if (!notification.success) return;
            invalidate(notification.path);
            if (notification.type == AssetType::Shader &&
            ...
            } else if (notification.type == AssetType::Mesh &&
                       !notification.removed &&
                       MESH_MANAGER.find(notification.path)) {
                (void)MESH_MANAGER.replace(notification.path);
            }
            if (changeListener_) {
                changeListener_(notification.path, notification.type,
                                notification.removed);
            }
        });
...
#if defined(MINI_RELEASE)
    Log::error("AssetManager", "Cooked Artifact is unavailable: %s",
               path.string().c_str());
    return false;
#else
    if (!ASSET_IMPORT_PIPELINE.initialized() ||
        !ASSET_IMPORT_PIPELINE.importAsset(path)) {
        Log::error("AssetManager", "Asset import failed: %s",
                   path.string().c_str());
        return false;
    }
...
std::shared_ptr<Asset> AssetManager::loadAsset(const VirtualPath& path) {
    if (!path.valid() || path.scheme() != "asset") {
        Log::error("AssetManager", "Invalid Asset path: %s",
                   path.string().c_str());
        return {};
    }
    if (auto existing = findCached(path)) return existing;
    if (!ensureImported(path)) return {};

    const auto record = ASSET_DATABASE.findByPath(path);
    const auto artifact = record ? loadAssetArtifact(record->artifactPath)
                                 : std::nullopt;
    if (!record || !artifact || artifact->assetId != record->id ||
        artifact->assetType != record->type) {
        Log::error("AssetManager", "Invalid Asset Artifact: %s",
                   path.string().c_str());
        return {};
    }
    ...
    case AssetType::Scene:
        asset = std::make_shared<SceneAsset>();
        break;
    default:
        Log::error("AssetManager", "Unsupported Asset type for: %s",
                   path.string().c_str());
        return {};
    }
    ...
    if (!asset->transfer(reader) || !reader.finished()) {
        Log::error("AssetManager", "Cannot deserialize Asset: %s",
                   path.string().c_str());
        return {};
    }
