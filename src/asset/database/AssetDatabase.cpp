...
[[nodiscard]] const char* importStatusName(AssetImportStatus status) {
    ...
    default: return "NotImported";
    }
}

[[nodiscard]] AssetImportStatus importStatusFromName(std::string_view name) {
    if (name == "Imported")
        return AssetImportStatus::Imported;
    if (name == "Failed")
        return AssetImportStatus::Failed;
    if (name == "Missing")
        return AssetImportStatus::Missing;
    return AssetImportStatus::NotImported;
}

[[nodiscard]] bool readUnsigned(const Json& object, const char* key, std::uint64_t& value) {
...
[[nodiscard]] std::optional<AssetRecord> parseRecord(const Json& value) {
    ...
    if (idValue == value.end() || !idValue->is_string() || typeValue == value.end() ||
        !typeValue->is_string() || sourceValue == value.end() || !sourceValue->is_string() ||
        metaValue == value.end() || !metaValue->is_string() || artifactValue == value.end() ||
        !artifactValue->is_string()) {
        return std::nullopt;
    }
    ...
    record.artifactPath = VirtualPath{artifactValue->get_ref<const std::string&>()};
    if (!record.id.valid() || record.type == AssetType::Unknown || !record.sourcePath.valid() ||
        !record.metaPath.valid() || !record.artifactPath.valid()) {
        return std::nullopt;
    }
    ...
    if (status == value.end() || !status->is_string() || error == value.end() ||
        !error->is_string() || dependencies == value.end() || !dependencies->is_array()) {
        return std::nullopt;
    }
...
std::optional<AssetRecord> AssetDatabase::findById(AssetId id) const {
    ...
    return found == records_.end() ? std::nullopt : std::optional<AssetRecord>{found->second};
}

std::optional<AssetRecord> AssetDatabase::findByPath(const VirtualPath& path) const {
    const auto id = assetIdFromPath(path);
    return id ? findById(*id) : std::nullopt;
}

std::optional<AssetId> AssetDatabase::assetIdFromPath(const VirtualPath& path) const {
    std::scoped_lock lock{mutex_};
    const auto found = pathIndex_.find(path.string());
    return found == pathIndex_.end() ? std::nullopt : std::optional<AssetId>{found->second};
}
...
bool AssetDatabase::addOrUpdate(AssetRecord record) {
    ...
    if (pathOwner != pathIndex_.end() && pathOwner->second != record.id) {
        Log::error("AssetDatabase",
                   "Asset path already has another ID: %s",
...
bool AssetDatabase::load() {
    ...
    if (root.is_discarded() || !root.is_object()) {
        Log::error("AssetDatabase", "Invalid database JSON: %s", databasePath().string().c_str());
        return false;
    }
    const auto version = root.find("version");
    const auto assets = root.find("assets");
    if (version == root.end() || !version->is_number_unsigned() ||
        version->get<std::uint32_t>() != 1 || assets == root.end() || !assets->is_array()) {
        Log::error(
            "AssetDatabase", "Unsupported database format: %s", databasePath().string().c_str());
        return false;
    }
...
bool AssetDatabase::save() const {
    ...
    std::ranges::sort(
        records, {}, [](const AssetRecord& record) { return record.sourcePath.string(); });

    Json assets = Json::array();
    for (const AssetRecord& record : records) {
        assets.push_back(serializeRecord(record));
    }
    const std::string content = Json{{"version", 1}, {"assets", std::move(assets)}}.dump(2) + '\n';
...
VirtualPath AssetDatabase::artifactPath(AssetId id) const {
    ...
    return directory.valid() ? directory.joined("asset.bin") : VirtualPath{};
}
