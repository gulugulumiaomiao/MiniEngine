...
AssetImportResult SceneAssetImporter::import(
    ...
    if (context.meta.assetType != AssetType::Scene ||
    ...
    const std::shared_ptr<SceneAsset> scene =
        detail::parseSceneAsset(context.sourcePath, *source);
    if (!scene) {
    ...
    dependencies.erase(std::ranges::unique(dependencies, {},
                                           &VirtualPath::string).begin(),
                       dependencies.end());

    BinaryWriter writer;
    if (!scene->transfer(writer)) {
        return fail("Cannot serialize SceneAsset: " +
                    context.sourcePath.string());
    }
    if (!FILE_SYSTEM.createDirectories(context.artifactPath.parent())) {
        return fail("Cannot prepare Scene Artifact: " +
                    context.artifactPath.string());
    }
    const AssetArtifact artifact{1, context.meta.assetId, AssetType::Scene,
                                 context.sourcePath, writer.takeBytes()};
    if (!saveAssetArtifact(context.artifactPath, artifact)) {
        return fail("Cannot save Scene Artifact: " +
                    context.artifactPath.string());
    }
    return AssetImportResult::succeeded(AssetType::Scene,
